...
def chrsix(x):
    if x.__class__ not in [int, long]:
        return x
    if PY3:
        return bytes([x])
    return chr(x)
...
KeyInfo = collections.namedtuple(
    "KeyInfo", "secret private_key public_key uncompressed_public_key addr wif compressed"
)
    ...
    if FillPool:
        ...
        for p in list_of_reserve_not_in_pool:
            update_wallet(
                db,
                "pool",
                {"public_key": binascii.unhexlify(p), "n": cpt, "nTime": ts(), "nVersion": 80100},
            )
            cpt += 1
...
def keyinfo(sec, network=None, print_info=False, force_compressed=None):
    ...
    r = KeyInfo(
        secret, private_key, ser_public_key, uncompressed_ser_public_key, addr, wif, compressed
    )
    if network:
...
class Xpriv(collections.namedtuple("XprivNT", "version depth prt_fpr childnr cc ktype key")):
    ...
    def ckd_xpriv(self, *indexes):
        ...
        child = self.__class__(
            self.version, self.depth + 1, hash_160(par_pubk)[:4], i, ir, 0, binascii.unhexlify(pk)
...
if __name__ == "__main__":
    ...
    parser.add_option("--passphrase", dest="passphrase", help="passphrase for the encrypted wallet")

    parser.add_option("--find_address", help="find info about an address")

    parser.add_option(
        "-d", "--dumpwallet", dest="dump", action="store_true", help="dump wallet in json format"
    )

    parser.add_option("--dumpformat", default="all", help="choose what to extract in a wallet dump")
    ...
    parser.add_option(
        "--datadir", dest="datadir", help="REMOVED OPTION: put full path in the --wallet option"
    )
    ...
    parser.add_option(
        "--label", dest="label", help="label shown in the adress book (defaults to '')", default=""
    )
    ...
    parser.add_option(
        "--namecoin", dest="namecoin", action="store_true", help="use namecoin address type"
    )
    ...
    parser.add_option(
        "--recov_size", dest="recov_size", help="number of bytes to read (e.g. 20Mo or 50Gio)"
    )
    ...
    parser.add_option(
        "--clone_watchonly_from", dest="clone_watchonly_from", help="path of the original wallet"
    )
    ...
    parser.add_option(
        "--random_key", action="store_true", help="print info of a randomly generated private key"
    )
    ...
    if options.minimal_encrypted_copy:
        ...
        assert not os.path.exists(os.path.join(db_dir, minimal_wallet)), (
            "There is already a minimal encrypted copy at %s/%s, exiting" % (db_dir, minimal_wallet)
        )
        kds = BCDataStream()
        vds = BCDataStream()
        encrypted_keys = []
    ...
    if options.find_address:
        addr_data = filter(
            lambda x: x["addr"] == options.find_address, json_db["keys"] + json_db["pool"]
        )
